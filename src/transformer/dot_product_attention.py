...
class DotProductAttention(nn.Module):
    ...
    def forward(
        ...
        x_kv = (
            x if x_kv is None else x_kv
        )  # if x_kv is not none cross attention will be used

        q = self.q_layer(x)
        k = self.k_layer(x_kv)
        v = self.v_layer(x_kv)

        raw_attention = torch.matmul(q, k.transpose(-1, -2)) / (
            math.sqrt(self.output_dim) if self.scaled else 1
        )
        ...
        if mask is not None:
            assert mask.shape == (
                ...
                x.shape[1],
                x_kv.shape[1],
            ), "Mask does not match attention shape"
            raw_attention += mask
